...
sources = get_github_docs("bryanschuetz", "d-ai-sources")
...
source_chunks = []
splitter = CharacterTextSplitter(separator=" ", chunk_size=1869, chunk_overlap=0)
for source in sources:
    for chunk in splitter.split_text(source.page_content):
        source_chunks.append(Document(page_content=chunk, metadata=source.metadata))

search_index = FAISS.from_documents(source_chunks, OpenAIEmbeddings())
template = """You are an AI writing assistant for the company DAI. Your job is respond to requests with generated text and sources that your users can easily integrate into their writing. ALWAYS return a "SOURCES" part in your answer. If you don't know the answer, just say that you don't know. Don't try to make up an answer. Respond to the following request: {question} {summaries}"""
