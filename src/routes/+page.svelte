  ...
  {#if loading === false}
      <section id="form-box" class="prose mb-8">
        ...
        <div>
          ...
          <p class="">Use existing 'Good News' text to generate <em>draft</em> project descriptions for use on the website and elsewhere. The model expects to see a <a href="https://gist.githubusercontent.com/BryanSchuetz/44fe7429b1dd83760b0b7f475666cb3b/raw/248e1c5caa9a3807abae99f08b02ba58206d626d/promt.txt">prompt formated like the one linked here</a>. A project name, followed by customer and contract info—then the rest of the text from the Good News email.</p>
        </div>
        </div>
          <form class="form"  action="?/submit" method="POST" enctype="multipart/form-data" use:enhance={handleSubmit} id="upload" name="upload">
              <div class="field">
                  <div class="control">
                      <textarea id="text-submission" name="text-submission" class="textarea form-textarea w-full h-[300px]" placeholder="Put The Good News Here" bind:value={textSubmission}></textarea>
                  </div>
              </div>
              <div class="field">
                  <div class="control">
